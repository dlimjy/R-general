...
modelset <- titan %>% select(Survived, Pclass, Sex, Age, SibSp, Parch, Fare, Embarked)
...
trainset$Embarked <- ifelse(trainset$Embarked == "", "S", trainset$Embarked)
trainset %>% nrow
testset %>% nrow()
